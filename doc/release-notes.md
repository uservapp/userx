UserX Core version *3.1.1* is now available from:  <https://github.com/uservapp/userx/releases>
...
Please report bugs using the issue tracker at github: <https://github.com/uservapp/userx/issues>

Non-Mandatory Update
==============

UserX Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.

How to Upgrade
==============

If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/UserX-Qt (on Mac) or userxd/userx-qt (on Linux).


Compatibility
==============

UserX Core is extensively tested on multiple operating systems using
...
UserX Core should also work on most other Unix-like systems but is not
...
### Fix spending for v1 zUSERX created before block 1050020

The transition to v2 zUSERX and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zUSERX to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zUSERX v1 will now be able to convert that into USERX and then zUSERX v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zUSERX stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by UserX stakers.
...
### Show progress percent for zuserx reindex operations
...
UserX Daemon & Client (RPC Changes)
--------------
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zUSERX code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zUSERX elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for zuserx reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zUSERX created before block 1050020. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/userx-translations/).
