...
// Copyright (c) 2019 The UserX developers
...
#if defined(NDEBUG)
#error "UserX cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uuserx) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zUSERX is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zUSERX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zUSERX without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zUSERX spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zUSERX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zUSERX spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zuserx");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zUSERX mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
	...
	} else if (nHeight < 26000) {
        nSubsidy = 0.1 * COIN;
    } else if (nHeight < 35000 && nHeight >= 26000) {
        nSubsidy = 8 * COIN;
        ...
        nSubsidy = 60 * COIN;
	} else if (nHeight < 170000 && nHeight >= 150000) {
        nSubsidy = 100 * COIN;
	} else if (nHeight < 220000 && nHeight >= 170000) {
        nSubsidy = 150 * COIN;
	} else if (nHeight < 250000 && nHeight >= 220000) {
        nSubsidy = 100 * COIN;
	} else if (nHeight < 270000 && nHeight >= 250000) {
        nSubsidy = 80 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZUSERXStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from UserX
...
{
    RenameThread("userx-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZUSERXMinted()
{
...
void RecalculateZUSERXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zUSERX supply
...
bool RecalculateUSERXSupply(int nHeightStart)
{
...
bool UpdateZUSERXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateUSERXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zUSERX money supply in the block index
    if (!UpdateZUSERXSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zUSERX supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zUserxSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    CAmount nExpectedMint = GetBlockValue(chainActive.Height() + 1);
    if (block.IsProofOfWork())
        nExpectedMint += nFees;
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert USERX to zUSERX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zUSERX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zUSERX serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zUSERX stake block is less than allowed start height", __func__);

    if (CZUserxStake* zUSERX = dynamic_cast<CZUserxStake*>(stake)) {
        CBlockIndex* pindexFrom = zUSERX->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zUSERX stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zUSERX stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zUSERX->GetValue()));
        if (nChecksum200 != zUSERX->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zUSERX->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZUSERX() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zUSERX fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zUSERX mints and %d zUSERX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // UserX: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
    // SPORK_14 is used for 70920
    if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
            return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 was used for 70920
