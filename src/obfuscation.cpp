    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat UserX and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 USERX per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeUserXAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100USERX+1 ( bit on if present )
    // bit 1 - 10USERX+1
    // bit 2 - 1USERX+1
    // bit 3 - .1USERX+1
    // bit 3 - non-denom
...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
            if (out.nValue == GetMNCollateral(chainActive.Height()) * COIN || out.nValue == GetMNCollateralOld(chainActive.Height()) * COIN) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("userx-obfuscation");
