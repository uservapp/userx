...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::USERX);
    ...
    if (!settings.contains("nAnonymizeUserXAmount"))
        settings.setValue("nAnonymizeUserXAmount", 1000);

    nAnonymizeUserXAmount = settings.value("nAnonymizeUserXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeUserXAmount"))
        SoftSetArg("-anonymizeuserxamount", settings.value("nAnonymizeUserXAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in userx.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUserXAmount:
            return QVariant(nAnonymizeUserXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUserXAmount:
            nAnonymizeUserXAmount = value.toInt();
            settings.setValue("nAnonymizeUserXAmount", nAnonymizeUserXAmount);
            emit anonymizeUserXAmountChanged(nAnonymizeUserXAmount);
            break;
        case CoinControlFeatures:
