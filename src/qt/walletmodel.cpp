...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered userx address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftTX doesn't support sending values that high yet. Transactions are currently limited to %1 USERX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal userx:URI (userx:XyZ...?message=example)
            {
...
static void NotifyzUSERXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzUSERXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzUSERXReset.connect(boost::bind(NotifyzUSERXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzUSERXReset.disconnect(boost::bind(NotifyzUSERXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zuserxTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
