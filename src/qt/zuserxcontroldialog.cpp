...
#include "ui_zuserxcontroldialog.h"
...
std::set<std::string> ZUserxControlDialog::setSelectedMints;
std::set<CMintMeta> ZUserxControlDialog::setMints;

ZUserxControlDialog::ZUserxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZUserxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZUserxControlDialog::~ZUserxControlDialog()
{
    delete ui;
}

void ZUserxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUserxControlDialog::updateList()
{
...
void ZUserxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUserxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZUserx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUserxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZUserxControlDialog::GetSelectedMints()
{
...
void ZUserxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
