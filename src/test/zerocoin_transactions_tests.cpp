...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zuserxTracker = unique_ptr<CzUSERXTracker>(new CzUSERXTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZUSERX_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZUSERX_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
