...
CzUSERXTracker::CzUSERXTracker(std::string strWalletFile)
{
...
CzUSERXTracker::~CzUSERXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzUSERXTracker::Init()
{
...
bool CzUSERXTracker::Archive(CMintMeta& meta)
{
...
bool CzUSERXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzUSERXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzUSERXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzUSERXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzUSERXTracker::GetSerialHashes()
{
...
CAmount CzUSERXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzUSERXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzUSERXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzUSERXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzUSERXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzUSERXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzUSERXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzUSERXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzUSERXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzUSERXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzUSERXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzUSERXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzUSERXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzUSERXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzUSERXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzUSERXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzUSERXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzuserx from DB\n", __func__, listDeterministicDB.size());
    }
