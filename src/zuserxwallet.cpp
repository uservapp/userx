...
CzUSERXWallet::CzUSERXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zuserx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZUSERXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZUSERXSeed_deprecated()) {
                    LogPrintf("%s: Updated zUSERX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zuserx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zuserx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzUSERXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZUSERXCount(nCountLastUsed);
    else if (!walletdb.ReadZUSERXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzUSERXWallet::Lock()
{
    seedMaster = 0;
}

void CzUSERXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzUSERXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZUSERX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzUSERXWallet::LoadMintPoolFromDB()
{
...
void CzUSERXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzUSERXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzUSERXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zuserxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzUSERXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZUSERX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zuserxTracker which also adds to database
    pwalletMain->zuserxTracker->Add(dMint, true);
...
void CzUSERXWallet::SeedToZUSERX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzUSERXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzUSERXWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZUSERXCount(nCountLastUsed);
...
void CzUSERXWallet::GenerateDeterministicZUSERX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzUSERXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzUSERXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
